...
export const actions = {
	getAttendance(
		context,
		{ company_id = '', startDate = '', endDate = '', showall = 1, search = '', defaultPage = false, checkins = '' }
	) {
		context.commit('setLoader');
		this.$axios
			.get(`/reportAttendance?company_id=${company_id}&startDate=${startDate}&endDate=${endDate}`)
			.then((resp) => {
	...
	getSalary(context, { company_id = '', showall = 1, search = '', defaultPage = false }) {
		this.$axios
			.get(`/reportSalary?company_id=${company_id}`)
			.then((resp) => {
	...
	getPermission(context, { company_id = '', showall = 1, search = '', defaultPage = false }) {
		this.$axios
			.get(`/reportPermission?company_id=${company_id}`)
			.then((resp) => {
	...
	getEmployee(context, { company_id = '', showall = 1, search = '', defaultPage = false }) {
		this.$axios
			.get(`/reportEmployee?company_id=${company_id}`)
			.then((resp) => {
